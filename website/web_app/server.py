...
INTERPRETATION_LABEL_PATTERN = re.compile(
    r"(?mi)^\s*(?:\*\*)?(Pathway description|Biological context|"
...
def require_team_login():
    ...
    if not AUTH_ENABLED or request.path in PUBLIC_PATHS:
        return None
...
def auth_me():
    ...
    if owner_id:
        try:
            payload['quota'] = get_quota_payload(owner_id)
...
def get_quota_payload(owner_id: str) -> dict:
    payload = quota_manager.status(owner_id)
...
def get_quota_status():
    ...
    try:
        return jsonify(get_quota_payload(owner_id))
...
def start_analysis():
    ...
    try:
        ...
        try:
            quota = quota_manager.reserve(owner_id)
        ...
        # Initial messages
        mode_text = " (Prompt Refinement Mode)" if use_iterative else ""
...
def export_data(session_id, fmt):
    ...
    if fmt == 'pdf':
        try:
            pdf_buffer = generate_full_export_pdf(session)
        except ImportError:
            return jsonify({
        ...
        response = send_file(
            ...
            as_attachment=True,
            download_name=f"GenePathwayAI_{safe_disease}_{session_id[:8]}.pdf",
            max_age=0,
        )
        ...
        for pw in pathways:
            ...
            writer.writerow([
                ...
                get_pathway_identifier(pw) or 'ID unavailable', category,
                ...
                pw.get('description', '')[:200], pw.get('cell_context', ''),
                ';'.join(str(pmid) for pmid in pw.get('cell_context_pmids', []) if pmid),
                pmids
            ])
        return jsonify({"data": output.getvalue(), "filename": f"pathways_{session.disease_name.replace(' ', '_')}_{session_id[:8]}.csv", "mime": "text/csv"})
        ...
        import json
        return jsonify({"data": json.dumps(export_obj, indent=2, ensure_ascii=False), "filename": f"analysis_data_{session.disease_name.replace(' ', '_')}_{session_id[:8]}.json", "mime": "application/json"})
...
def run_real_pathway_analysis(session: AnalysisSession) -> list:
    ...
    try:
        ...
        <h4>📊 Hypothesis-level validation</h4>
...
def gpt_predict_pathways_with_context(genes: list, disease_name: str, disease_description: str, retained_pathways: list, iteration: int = 2) -> tuple:
    """
...
def gpt_rank_pathways_direct(enrichment_df, disease_name, disease_description, top_n=30):
    """
...
def gpt_rank_pathways_simple(pathways_df, disease_name, disease_description, query_agent, framework, batch_size=20, top_n=200):
    """
...
{query}"""
            ...
            if answer and len(answer.strip()) > 10:
                return {
                    "answer": answer,
                    "data": {"type": "gpt_response", "model": GPT_MODEL}
                }
            else:
...
def generate_full_export_pdf(session: AnalysisSession) -> BytesIO:
    """Build a paginated, source-linked full analysis report."""
    ...
    def report_page(canvas, document):
        ...
        canvas.drawString(15 * mm, 8.5 * mm, 'GenePathwayAI / Full analysis report')
        canvas.drawRightString(width - 15 * mm, 8.5 * mm, f"Page {document.page}")
        canvas.restoreState()

    story = []
    story.append(Paragraph('GenePathwayAI', styles['subtitle']))
    story.append(Paragraph('Full pathway analysis report', styles['title']))
    ...
    story.append(Paragraph(
        ...
        f"<b>Model:</b> {safe_text(GPT_MODEL)}",
        styles['subtitle'],
    ))
    ...
    metric_data = [[
        ...
        Paragraph(str(run_summary['literature_records']), styles['metric']),
    ], [
        ...
        Paragraph('unique PubMed articles', styles['metric_label']),
    ]]
    metrics = Table(metric_data, colWidths=[doc.width / 4] * 4, rowHeights=[11 * mm, 9 * mm])
    ...
    validation_comparison = session.results.get('validation_comparison') or {}
    prompt_rounds = validation_comparison.get('rounds') or []
    if prompt_rounds:
        story.append(Paragraph('Prompt-pass output', styles['h2']))
        has_matched = any(
            isinstance(round_data, dict) and round_data.get('matched_hypotheses') is not None
            for round_data in prompt_rounds
        )
        has_validated = any(
            isinstance(round_data, dict)
            and round_data.get('statistically_validated_hypotheses') is not None
            for round_data in prompt_rounds
        )
        round_headers = ['Prompt pass', 'Generated hypotheses']
        if has_matched:
            round_headers.append('Matched hypotheses')
        if has_validated:
            round_headers.append('Unique validated')
        round_rows = [[
            Paragraph(label, styles['table_header']) for label in round_headers
        ]]
        for round_index, round_data in enumerate(prompt_rounds, 1):
            if not isinstance(round_data, dict):
                continue
            round_row = [
                Paragraph(str(round_data.get('round') or round_index), styles['table_cell']),
                Paragraph(str(round_data.get('generated_hypotheses', 'N/A')), styles['table_cell']),
            ]
            if has_matched:
                round_row.append(Paragraph(str(round_data.get('matched_hypotheses', 'N/A')), styles['table_cell']))
            if has_validated:
                round_row.append(Paragraph(str(round_data.get('statistically_validated_hypotheses', 'N/A')), styles['table_cell']))
            round_rows.append(round_row)
        if len(round_rows) > 1:
            column_count = len(round_headers)
            round_table = Table(
                round_rows,
                repeatRows=1,
            ...
            story.extend([round_table, Spacer(1, 3 * mm)])

    story.append(Paragraph('1. Ranked statistically validated pathways', styles['h1']))
    table_rows = [[
    ...
    pathway_table = Table(
        table_rows,
        repeatRows=1,
        colWidths=[8 * mm, 48 * mm, 25 * mm, 18 * mm, 23 * mm, 18 * mm, 20 * mm],
        hAlign='LEFT',
    )
    ...
    story.append(Paragraph('2. Pathway evidence and interpretation', styles['h1']))
    for index, pathway in enumerate(pathways, 1):
        ...
        title = f"{index}. {safe_text(pathway.get('name') or 'Unnamed pathway')}"
        if url:
            title += f" <font color=\"#1f9d83\"><link href=\"{url}\">{safe_text(pathway_id)}</link></font>"
        else:
            title += f" ({safe_text(pathway_id)})"
        ...
        if cell_context:
            ...
            if context_pmids:
                context_links = ', '.join(
                    ...
                    for pmid in context_pmids
                )
                story.append(Paragraph(
                    f'<b>Related pathway literature:</b> {context_links}',
                    styles['small'],
                ))
    ...
    story.append(PageBreak())
    story.append(Paragraph('3. Input gene appendix', styles['h1']))
    story.append(Paragraph(
...
---
*Generated by Gene Pathway Analysis Web Application*  
*Model: {GPT_MODEL} | Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*
"""
...
def run_iterative_pathway_analysis(session: AnalysisSession) -> list:
    ...
    for iteration in range(1, 3):
        ...
        session.add_message("system", f"🔄 Prompt Refinement: Round {iteration}/2 ({round_label}) {'─' * 40}")
    ...
    pathways_list = convert_pathways_to_output_format(ranked_pathways, session.disease_name)
    ...
    dist_str = ", ".join([f"{cat}: {count}" for cat, count in sorted(cat_dist.items())])
    session.add_message("system", 
        f"✅ **Prompt Refinement Complete**: {len(pathways_list)} pathways | {dist_str}")
...
def display_iteration_stats(session, iteration, category_stats, retained_count, 
    ...
    <h4>📊 Round {iteration}/2 Statistics ({'Initial Prompt' if iteration == 1 else 'Refined Prompt'})</h4>
    <table class="pathway-table stats-table">
        <thead>
            <tr>
                <th>Category</th>
...
def convert_pathways_to_output_format(pathways_df, disease_name='the selected disease'):
